...
raw<-read.xlsx('all.xlsx',sheet = 3,rowNames = T,check.names = F,sep.names = " ")
...
samples<-samples[,-1]
temps<-'WGCNA_Analysis - pearson'
if(!dir.exists(temps)){dir.create(temps)}
# write.xlsx(annodata,paste0(temps,'/analysis.xlsx'),rowNames=T)
...
sft<-pickSoftThreshold(datExpr, powerVector = powers, verbose = 5,corOptions = list(use = "p",method='pearson'))#,corOptions = list(use = "p",method='pearson')
...
temp0<-paste0(temps,'/3 Weight_Co-expression_Network')
if(!dir.exists(temp0)){dir.create(temp0)}
adjacency = adjacency(datExpr, power = sft$powerEstimate,corOptions = list(use = "p",method = "spearman"))
# adjacency = adjacency(datExpr, power = 8) 
...
dev.off()
# 调用函数进行模块合并
merge = mergeCloseModules(datExpr, dynamicColors, cutHeight = MEDissThres, getNewUnassdME = T, getNewMEs = T)
# 模块颜色
...
moduleTraitCor = cor(MEs, samples , use = "p",method = 'spearman')
write.xlsx(moduleTraitCor,paste0(temp0,'/corr.xlsx'),rowNames=T)
write.xlsx(MEs,paste0(temp0,'/Mes.xlsx'),rowNames=T)
...
for (gg in colnames(mes)) {
  ...
  p<-ggplot()+
    geom_bar(aes(y=mes[,gg],x=factor(rownames(mes),levels=rownames(mes))),stat = 'identity',color=str_remove(gg,'ME'),fill=c)+
    xlab('Trait')+
...
for (kk in unique(moudledata$mergedColors)) {
  ...
  ##模块内基因绘制聚类树
  ###发现对数据标量化，聚类效果反而会更好
  moudlesampleTree = hclust(dist(scale(datExpr[,moudlegene])), method = "complete")
  ...
  limits<-floor(min(max(t(scale(datExpr[,moudlegene]))),
                    -1*min(t(scale(datExpr[,moudlegene]))))*100000000000)/100000000000
  bk <- c(seq(-1*limits,-0.0001,by=0.0001),seq(0,limits,by=0.0001))
...
for (kk in unique(moudledata$mergedColors)) {
  ...
  ###hubgene绘制热图
  limits<-floor(min(max(t(scale(hubgene))),-1*min(t(scale(hubgene))))*100000000000)/100000000000
...
limits<-floor(min(max(t(scale(hubgenes))),-1*min(t(scale(hubgenes))))*100000000000)/100000000000
bk <- c(seq(-1*limits,-0.1,by=0.01),seq(0,limits,by=0.01))
c<-c(colorRampPalette(colors = c("blue","white"))(length(bk)/2),colorRampPalette(colors = c("white","red"))(length(bk)/2))
...
for (kk in unique(moudledata$mergedColors)) {
  ...
  ######################################绘制基因网络草图############################
  Edge = read.table(paste(temp1,"/CytoscapeInput-edges-", kk, ".txt", sep=""),header = 1,sep = '\t',quote = "")
