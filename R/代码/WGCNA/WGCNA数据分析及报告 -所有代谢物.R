...
raw<-read.xlsx('SLE.xlsx',rowNames = T)
...
sampleTree = hclust(dist(scale(datExpr)), method = "complete")
...
sft<-pickSoftThreshold(datExpr, powerVector = powers, verbose = 5,corOptions = list(use = "p",method='spearman'))
...
for (kk in unique(moudledata$mergedColors)) {
  ...
  limits<-floor(min(max(t(scale(datExpr[,moudlegene]))),
                    -1*min(t(scale(datExpr[,moudlegene]))))*100000)/100000
...
for (kk in unique(moudledata$mergedColors)) {
  ...
  ###hubgene绘制热图
  limits<-floor(min(max(t(scale(hubgene))),-1*min(t(scale(hubgene))))*100000)/100000
  bk <- c(seq(-1*limits,-0.1,by=0.01),seq(0,limits,by=0.01))
  c<-c(colorRampPalette(colors = c("blue","white"))(length(bk)/2),colorRampPalette(colors = c("white","red"))(length(bk)/2))
...
limits<-floor(min(max(t(scale(hubgenes))),-1*min(t(scale(hubgenes))))*100000)/100000
bk <- c(seq(-1*limits,-0.1,by=0.01),seq(0,limits,by=0.01))
c<-c(colorRampPalette(colors = c("blue","white"))(length(bk)/2),colorRampPalette(colors = c("white","red"))(length(bk)/2))
