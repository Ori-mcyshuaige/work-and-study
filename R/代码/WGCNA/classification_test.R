...
### 数据读取与预处理
raw<-read.table('classification_test.csv',sep = ",",header = T)
rownames(raw)<-paste0(raw$pred,'_',rownames(raw))
# rownames(raw)<-sapply(1:nrow(raw), function(i){paste0(raw[i,c('platform','id')],collapse = '_')})
data<-as.data.frame(t(raw[,1:6000]))#[,grep('Control|Before|After',colnames(raw))]
