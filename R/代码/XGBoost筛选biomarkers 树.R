...
if (length(bios$Feature)>1) {
  ...
  liqundata<-liqundataraw[, bios$Feature]
  ...
  numtogroups<-as.numeric(unique(traindataraw$Group))-1
  names(numtogroups)<-unique(traindataraw$Group)
  liqundata1<-Matrix(data.matrix(liqundata[,-ncol(liqundata)]),sparse=T)
  newliqun<-xgb.DMatrix(data=liqundata1,label = unlist(sapply(liqundata$Group, function(i){numtogroups[[i]]})))
  if (length(unique(unlist(sapply(rownames(liqundataraw),function(i){substring(paste0(str_extract_all(i,'\\D')[[1]],collapse = ''),1,2)}))))>1){
    liqundata$Group<-factor(liqundata$Group,levels = unique(liqundata$Group))
    liqundataroc<-roc(as.numeric(liqundata$Group)-1,predict(xgboostmodelnew,newdata = newliqun),ci=T,percent=TRUE)
  }
  getclassification(newliqun,liqundata,output=paste0('XGBoost-tree/','4 ROC curve/','离群.csv'))
