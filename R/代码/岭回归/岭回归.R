...
setwd("D:/马驰宇/桌面/何院专辑/研究院项目/戴主任项目重分析/氧化脂质/新数据补值/氧化脂质/加入临床指标/岭回归")
...
data1<-read.xlsx("氧化脂质预测SLEDAI-原始数据.xlsx",sheet = 3,colNames = T,rowNames = T,check.names = F,sep.names = ".")###预测
data2<-read.xlsx("氧化脂质预测SLEDAI-原始数据.xlsx",sheet = 2,colNames = T,rowNames = T,check.names = F,sep.names = ".")
...
for(i in colnames(data1)){
  ...
  data$Group<-unlist(sapply(rownames(data),function(i){paste0(strsplit(i,'\\d')[[1]][1],collapse = '')}))
