...
raw<-read.xlsx("data.xlsx",sheet = 1,startRow = 1,colNames = TRUE,rowNames = TRUE)
...
if (length(bios)>1) {
  ...
  dev.off()
  # if (!dir.exists(paste0('Lasso-logistic/','4 ROC curve'))){dir.create(paste0('Lasso-logistic/','4 ROC curve'))}
  # rocs<-roc(as.numeric(testdata$Group)-1,predict(cv.fittest,newx = as.matrix(testdata[,bios]),type='response'),ci=T,percent=TRUE)
  # png(paste0('Lasso-logistic/','4 ROC curve/','ROC plot.png'))
  # plot.roc(rocs,print.thres=TRUE,print.auc=TRUE,col='red',reuse.auc=FALSE)
  # dev.off()
  # pdf(paste0('Lasso-logistic/','4 ROC curve/','ROC plot.pdf'))
  # plot.roc(rocs,print.thres=TRUE,print.auc=TRUE,col='red',reuse.auc=FALSE)
  # dev.off()
  # 
  # 
  ...
  # }
  # validationdata<-validationraw[, bios]
  # # validationdata<-data.frame(log(validationdata))
  # validationdata<-data.frame(scale(validationdata))
  ...
  # 
  # 
  # rocvalidationdata<-data.frame(y=predict(cv.fittest,newx = as.matrix(validationdata[,bios]),type='class'),sex=validationraw$sex)
  # colnames(rocvalidationdata)[1]<-'prediction'
  # rocvalidationdata$true<-validationdata$Group
  # write.csv(rocdata,file=paste0('Lasso-logistic/','4 ROC curve/','Test samples.csv'))
  # roctest<-roc(as.numeric(validationdata$Group)-1,predict(cv.fittest,newx = as.matrix(validationdata[,bios]),type='response'),ci=T,percent=TRUE)
  # # roctestnan<- roc(roctestdata$true[testraw$sex=='男'],roctestdata$prediction[testraw$sex=='男'],ci=T,percent=TRUE)
  # # roctestnv<- roc(roctestdata$true[testraw$sex=='女'],roctestdata$prediction[testraw$sex=='女'],ci=T,percent=TRUE)
  # png(paste0('Lasso-logistic/','4 ROC curve/','Test samples ROC plot.png'))
  # plot.roc(roctest,print.thres=TRUE,print.auc=TRUE,col='red',reuse.auc=FALSE)
  # # plot.roc(roctest,print.thres=TRUE,print.auc=TRUE,col='green',reuse.auc=FALSE,add = T)
  # # plot.roc(roctest,print.thres=TRUE,print.auc=TRUE,col='orange',reuse.auc=FALSE,add = T)
  # dev.off()
  # pdf(paste0('Lasso-logistic/','4 ROC curve/','Test samples ROC plot.pdf'))
  # plot.roc(roctest,print.thres=TRUE,print.auc=TRUE,col='red',reuse.auc=FALSE)
  # dev.off()
}
file.copy("Lasso-logistic回归筛选biomarker.R",
          ...
          overwrite = T)
# predtest <- prediction(roctestdata$prediction,roctestdata$true)   #预测值(0.5二分类之前的预测值)和真实值     
# performance(predtest,'auc')@y.values        #AUC值  
# predtest <- performance(predtest,'tpr','fpr')  
# plot(predtest) 
